...
class CPIH(Agent):
    ...
    def controller(self):
        ...
        for name, neighbor in self.neighbor_position.items():
            X[i] = np.array(neighbor)+2*(np.random.rand(1,2)-np.array(([-.5,-.5])))*imp
        ...
        Bx = self.getImprecisionRegions(X,imp)
        tc = TukeyContour(X)
        safepoints = tc.CPIH_Safepoint(Bx,6,5)
 
        if safepoints.shape[0] > 0:
            # Target is the centroid of the median contour
            
            safepoint = np.mean(safepoints, axis=0)
