...
if [ $# -lt 2 ]; then
  ...
  echo "Operations: copy, sync, build, deepBuild, run, stop, pullLogs, clearLogs, calibrate, testCalibrate, test"
...
OS_TYPE="$(uname)"
open_terminal() {
  local cmd="cd $(pwd) && ./scripts/${operation}.sh $USER $pi_ip $pi_wrk_space $robot_num $@"

  if [[ "$OS_TYPE" == "Darwin" ]]; then
    # macOS
    osascript <<EOF
tell application "Terminal"
    do script "bash -c '$cmd'"
end tell
EOF
